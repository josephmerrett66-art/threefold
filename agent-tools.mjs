...
export function registerGameTools(api){
 ...
 const definitions=[
  ...
  {name:'place_settlement_building',title:'Place a building',description:'Reserve resources and place one building or path at integer world coordinates. Settlers perform construction.',inputSchema:{type:'object',properties:{type:{type:'string',enum:['shelter','farm','stockpile','well','workshop','path']},x:{type:'integer',minimum:1,maximum:74},y:{type:'integer',minimum:1,maximum:74}},required:['type','x','y'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute:input=>{if(!input||!['shelter','farm','stockpile','well','workshop','path'].includes(input.type)||!Number.isInteger(input.x)||!Number.isInteger(input.y))throw Error('Supply a valid building type and integer x/y coordinates.');return api.place(input.type,input.x,input.y);}},
